...
// Shadertoy uniforms (you must declare these)
uniform float iTime;
uniform vec3 iResolution;
uniform vec2 iMouse;
uniform sampler2D iChannel0;  // if needed

out vec4 FragColor;

// Your Shadertoy code (defines, functions, etc.)
#define T iTime
...
vec3 c(float t) {
    return clamp(vec3(0.455, 0.322, 0.216) + vec3(-0.073, 0.119, 0.150) * cos(6.28318 * (vec3(20.000, 1.000, 1.000) * t + vec3(0.100, -0.256, -0.231))), 0.0, 1.0);
}
